  ...
  try {
    ...
    if (!bookingNumber && !email) {
      return NextResponse.json(
        {
          success: false,
          message: 'Please provide either a booking number (e.g., WWT-2024-001234) or the email address used for the booking.',
          booking: null
        },
    ...
    if (result.rows.length === 0) {
      ...
      return NextResponse.json(
        {
          success: false,
          message: `I couldn't find a booking with ${searchType}. Please double-check the information or contact us at info@wallawalla.travel for assistance.`,
          booking: null
        },
    ...
    console.error('GPT booking-status error:', error);
    return NextResponse.json(
      {
        success: false,
        message: 'Unable to look up booking status at this time. Please try again or contact us at info@wallawalla.travel.',
        booking: null
      },
