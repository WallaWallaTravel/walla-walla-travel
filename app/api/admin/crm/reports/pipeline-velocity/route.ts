...
export const GET = withAdminAuth(async (request: NextRequest, _session) => {
  ...
  const avgTimeToCloseResult = await prisma.$queryRawUnsafe<{
  ...
  const stageVelocityResult = await prisma.$queryRawUnsafe<{
    stage_name: string;
    stage_color: string;
    sort_order: number;
  ...
  const stageConversionResult = await prisma.$queryRawUnsafe<{
    from_stage: string;
    to_stage: string;
  ...
  const healthMetricsResult = await prisma.$queryRawUnsafe<{
  ...
  const staleDealsResult = await prisma.$queryRawUnsafe<{
    deal_id: number;
    deal_title: string;
    stage_name: string;
  ...
  const monthlyFlowResult = await prisma.$queryRawUnsafe<{
    month: string;
  ...
  return NextResponse.json({
    ...
    data: {
      time_to_close: {
        avg_days: avgTimeToCloseResult[0]?.avg_days_to_close || 0,
        deals_won: avgTimeToCloseResult[0]?.deals_won || 0,
        total_value: avgTimeToCloseResult[0]?.total_value_won || 0,
      },
      stage_velocity: stageVelocityResult,
      stage_conversions: stageConversionResult,
      health_metrics: healthMetricsResult[0],
      stale_deals: staleDealsResult,
      monthly_flow: monthlyFlowResult,
    },
  });
});
