...
export const GET = withAdminAuth(
  async (_request: NextRequest, _session, context) => {
    ...
    const partnerRows = await prisma.$queryRaw<Array<Record<string, unknown>>>`
    ...
    if (partner.winery_id) {
      const wineryRows = await prisma.$queryRaw<Array<Record<string, unknown>>>`
        SELECT id, name, slug, short_description, description, meta_title, meta_description,
                ...
                keywords, logo_url
         FROM wineries WHERE id = ${partner.winery_id}`;
      linkedWinery = wineryRows[0] || null;

      wineryContent = await prisma.$queryRaw<Array<{ content_type: string; title: string; content: string }>>`
        SELECT content_type, title, content
         FROM winery_content WHERE winery_id = ${partner.winery_id}`;
    }

    // Count insider tips
    let insiderTipsCount = 0;
    if (partner.winery_id) {
      const tipsRows = await prisma.$queryRaw<Array<{ count: string }>>`
        SELECT COUNT(*)::text as count FROM winery_insider_tips WHERE winery_id = ${partner.winery_id}`;
      insiderTipsCount = parseInt(tipsRows[0]?.count || '0');
    }

    // Count photos
    let photoCount = 0;
    if (partner.winery_id) {
      const photoRows = await prisma.$queryRaw<Array<{ count: string }>>`
        SELECT COUNT(*)::text as count FROM winery_media WHERE winery_id = ${partner.winery_id}`;
      photoCount = parseInt(photoRows[0]?.count || '0');
    }
