...
export const GET = withAdminAuth(
  async (_request: NextRequest, _session, context) => {
    ...
    const partnerRows = await prisma.$queryRawUnsafe<Record<string, any>[]>(
    ...
    if (partner.winery_id) {
      ...
      wineryContent = await prisma.$queryRawUnsafe<Array<{ content_type: string; title: string; content: string }>>(
    ...
    if (partner.winery_id) {
      const tipsRows = await prisma.$queryRawUnsafe<Array<{ count: string }>>(
    ...
    let photoCount = 0;
    if (partner.winery_id) {
      const photoRows = await prisma.$queryRawUnsafe<Array<{ count: string }>>(
