...
export const POST = withErrorHandling(async (request: NextRequest) => {
  ...
  if (timeCardResult.rows.length === 0) {
    throw new NotFoundError('Time card not found');
  }

  const timeCard = timeCardResult.rows[0];

  if (timeCard.clock_out_time !== null) {
    throw new BadRequestError('Cannot assign vehicle - shift is already completed');
  }
  ...
  if (vehicle.status === 'out_of_service') {
    throw new BadRequestError('Vehicle is out of service');
  }
