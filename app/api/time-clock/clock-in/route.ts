...
import { z } from 'zod';
import { withErrorHandling, BadRequestError, ConflictError, ForbiddenError } from '@/lib/api/middleware/error-handler';
...
export const POST = withErrorHandling(async (request: NextRequest) => {
  ...
  if (existingResult.rows.length > 0) {
    throw new ConflictError('Driver is already clocked in today');
  }
  ...
  if (vehicleInUseResult.rows.length > 0) {
    throw new ConflictError(`Vehicle is already in use by ${vehicleInUseResult.rows[0].driver_name}`);
  }
