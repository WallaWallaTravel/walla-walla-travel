...
export const POST = withCSRF(
  ...
  if (proposal.status !== 'accepted') {
    throw new BadRequestError(`Cannot convert proposal with status: ${proposal.status}. Only accepted proposals can be converted.`);
  }

  // Check if already converted
  if (proposal.converted_to_booking_id) {
    throw new BadRequestError('This proposal has already been converted to a booking');
  }
