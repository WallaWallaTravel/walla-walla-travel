...
export async function POST(
  ...
  try {
    ...
    if (original.status !== 'declined') {
      return NextResponse.json(
        { success: false, error: `Cannot create counter for proposal with status: ${original.status}. Only declined proposals can be countered.` },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to create counter-proposal' },
      { status: 500 }
    );
  } finally {
    await pool.end();
...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to fetch negotiation history' },
      { status: 500 }
    );
  } finally {
    await pool.end();
