 ...
 * Request a password reset email for hotel partners
...
export const POST = withCSRF(
  ...
  withErrorHandling(async (request: NextRequest) => {
    ...
    // Look up hotel partner (but always return success regardless)
    ...
    if (hotelResult.rows.length > 0) {
      ...
      const resetUrl = `${appUrl}/partner-portal/reset-password?token=${rawToken}`;
