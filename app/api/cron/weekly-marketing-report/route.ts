...
function buildReportEmail(data: WeeklyData, aiSummary: string, unsubscribeUrl?: string): string {
  ...
  const topPostHtml = data.topPost
    ? `
      <div class="em-card" style="background-color: #fef3c7; border: 1px solid #f59e0b; border-radius: 8px; padding: 16px; margin: 16px 0;">
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f3f4f6;">
  <div class="em-wrapper" style="max-width: 600px; margin: 0 auto; background: #ffffff;">
    ...
    <div style="padding: 0 24px 24px;">
      ...
      <div class="em-card" style="background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 16px;">
        <p style="margin: 0 0 12px 0; font-size: 14px; line-height: 1.6; color: #334155;">${aiSummaryHtml}</p>
      </div>
    </div>

    <!-- Footer -->
    <div class="em-footer" style="background-color: #f8fafc; padding: 20px 24px; text-align: center; border-top: 1px solid #e2e8f0;">
      ...
      <p style="margin: 4px 0 0 0; font-size: 12px; color: #94a3b8;">Report date: ${new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })}</p>${unsubscribeUrl ? `
...
export const GET = withCronAuth('weekly-marketing-report', async (_request: NextRequest) => {
  ...
  try {
    ...
    // 3. CAN-SPAM: Check if admin has unsubscribed
    const adminEmail = process.env.ADMIN_EMAIL || 'info@wallawalla.travel'
    if (await emailPreferencesService.isUnsubscribed(adminEmail)) {
      logger.info('Skipping weekly marketing report — recipient unsubscribed', { email: adminEmail })
      return NextResponse.json({
        success: true,
        message: 'Report skipped — recipient unsubscribed',
        skipped: true,
      })
    }

    // Get unsubscribe token and headers
    const pref = await emailPreferencesService.getOrCreatePreference(adminEmail)
    ...
    // 4. Build email HTML
    const emailHtml = buildReportEmail(weeklyData, aiSummary, unsubscribeUrl)
