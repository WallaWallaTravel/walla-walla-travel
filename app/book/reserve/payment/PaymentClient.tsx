...
/**
 * Stripe Payment Client Component
 * Receives reservation data as props from Server Component parent.
 * Handles Stripe Elements setup, payment intent creation, and payment confirmation.
 */
...
export function PaymentClient({ reservation }: { reservation: ReservationData }) {
  ...
  const { trackBookingProgress } = useBookingTracking();
  ...
  const handlePaymentSuccess = async (paymentIntentId: string) => {
    try {
      ...
      router.push(`/book/reserve/confirmation?id=${reservation.id}`);
    } catch (err: unknown) {
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between text-lg">
              ...
              <span className="font-bold text-gray-900">${reservation.deposit_amount.toFixed(2)}</span>
            </div>
          </div>
        </div>
