...
import { getDashboardStats, isAuthenticated, logout, getDriver } from '@/lib/api'
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        setStats({
          activeDrivers: dashboardStats.active_trips || 0,
          pendingDocs: dashboardStats.pending_docs || 0,
          expiringDocs: dashboardStats.expiring_docs || 0,
          todaysBookings: dashboardStats.todays_bookings || 0
        })
      } catch (err) {
    ...
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="px-4 mb-4">
        ...
        <div className="space-y-2">
          ...
          <button 
            onClick={() => router.push('/workflow/client-notes')}
            className="w-full bg-gray-800 text-white py-3 rounded-lg font-medium hover:bg-gray-700"
          >
            📝 Client Notes
          </button>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-300">
        <div className="grid grid-cols-5 py-2">
          ...
          <button 
            onClick={() => router.push('/workflow/daily')}
