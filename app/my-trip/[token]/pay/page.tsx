...
function PaymentForm({
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const { error: confirmError } = await stripe.confirmPayment({
      elements,
      confirmParams: {
        return_url: `${window.location.origin}/my-trip/${token}/pay/success`,
      },
    });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <button
        ...
        className="w-full bg-indigo-600 text-white px-8 py-4 rounded-lg font-bold text-lg hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-lg hover:shadow-xl flex items-center justify-center gap-2"
      >
        {processing ? (
          <>
            <svg
...
export default function MyTripPaymentPage() {
  ...
  const initializePayment = async () => {
    try {
      // Fetch proposal details via access token
      ...
      if (prop.status !== 'accepted') {
        throw new Error('This trip must be accepted before payment');
      }

      // Check if already paid
      if (prop.deposit_paid) {
        throw new Error('The deposit for this trip has already been paid');
      }

      // Create payment intent
      const paymentResponse = await fetch(
        `/api/my-trip/${token}/create-payment`,
        {
          method: 'POST',
