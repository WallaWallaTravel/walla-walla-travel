...
export async function getConversionFunnel(days: number = 30): Promise<{
  ...
  const result = await prisma.$queryRawUnsafe<Array<{
    sessions_with_queries: bigint
    sessions_with_clicks: bigint
    sessions_with_bookings: bigint
  }>>(`
    SELECT
  ...
  return {
    visits: Number(row.sessions_with_queries) || 0,
    queries: Number(row.sessions_with_queries) || 0,
    clicks: Number(row.sessions_with_clicks) || 0,
    bookings: Number(row.sessions_with_bookings) || 0
  }
}
