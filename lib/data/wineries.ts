...
export const getWineryNarrativeContent = cache(async (wineryId: number): Promise<WineryNarrativeContent> => {
  try {
    ...
    const contentRows = await prisma.$queryRawUnsafe<WineryContent[]>(
    ...
    const tipsRows = await prisma.$queryRawUnsafe<InsiderTip[]>(
    ...
    return {
      originStory: contentRows.find(c => c.content_type === 'origin_story') || null,
      philosophy: contentRows.find(c => c.content_type === 'philosophy') || null,
      uniqueStory: contentRows.find(c => c.content_type === 'unique_story') || null,
      insiderTips: tipsRows,
    };
  } catch (error) {
