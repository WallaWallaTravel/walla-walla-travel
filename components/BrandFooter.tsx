 ...
 * Used in proposal and payment flows to show brand-specific contact info
 */
import Link from 'next/link';
import { getBrandEmailConfig, BrandEmailConfig } from '@/lib/email-brands';

interface BrandFooterProps {
  brandId?: number;
}

export default function BrandFooter({ brandId }: BrandFooterProps) {
  const brand: BrandEmailConfig = getBrandEmailConfig(brandId);

  // Get dialable phone number (remove formatting)
  const dialablePhone = brand.phone.replace(/[^+\d]/g, '');
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
          ...
          <div>
            ...
            <p className="text-gray-600 text-sm">{brand.website}</p>
          </div>
            ...
            <div className="space-y-3">
              ...
              >
                ...
                <div>
                  ...
                  <div className="font-semibold">{brand.reply_to}</div>
                </div>
              </a>
            </div>
          </div>
