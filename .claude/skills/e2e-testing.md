---
name: e2e-testing
description: End-to-end test workflows for critical business paths. Use when writing Playwright tests or verifying user flows.
---
...
- Navigate to /admin/trip-proposals/new
...
- Verify: confirmation email triggered (check email logs)
...
## Notes
- Playwright foundation is in progress (tracked in project backlog)
- Chrome MCP verification (navigate, read_network_requests, get_page_text) is done in the Claude.ai Chat/Projects interface, not in Claude Code
- See monitoring.md for Chrome MCP verification patterns
